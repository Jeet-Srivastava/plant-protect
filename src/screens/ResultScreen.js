  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <View style={styles.card}>
        ...
        <Text style={styles.label}>Recommended treatments / preventive actions</Text>
        {treatments.map((t, idx) => (
          <Text key={idx} style={styles.body}>• {t}</Text>
        ))}
      </View>
      <Button title="Back to Scan" onPress={() => navigation.popToTop()} />
    </ScrollView>
  );
}
...
const styles = StyleSheet.create({
  ...
  card: { padding: 16, borderWidth: 1, borderColor: '#eee', borderRadius: 12, backgroundColor: '#fff', gap: 8 },
