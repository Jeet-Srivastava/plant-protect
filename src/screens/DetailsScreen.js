...
export default function DetailsScreen({ route }) {
  ...
  if (!record) return <View style={styles.center}><Text>Record not found.</Text></View>;
  ...
  return (
    <ScrollView contentContainerStyle={{ padding: 16, gap: 12 }}>
      ...
      <Text style={styles.h2}>{record.diseaseName} • {Math.round(record.confidence*100)}%</Text>
      <Text style={styles.body}>{record.description}</Text>
      <Text style={styles.h3}>Treatments / Preventive Actions</Text>
      {record.treatments.map((t, idx) => (<Text key={idx} style={styles.body}>• {t}</Text>))}
    </ScrollView>
  );
}
